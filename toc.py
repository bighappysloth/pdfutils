...
merge_parser.add_argument('-inherit_toc', type=bool, nargs = 1, required=False, help='Inherit TOC from subfile')
merge_parser.add_argument('-add_blank_page_after_subfile', '-add_blank', action='store_true', help='Add Blank Page after each subfile? (uses size of last page)')
...
def marginManager(path,margin,sf,z):
	with open(path,'rb') as f:
		...
		for i in range(len(p.pages)):
			...
			print('\tpagenum: ' + str(i) + ' size: ' + str(w) + '/' + str(h) + '=' + str(w/h))
			new_page = writer.addBlankPage(
			w*sf + 2*margin,
			h*sf + z*margin
			)
			translation = Transformation().translate(tx=float(margin),ty=float(z/2.)*margin)
			page.add_transformation(translation)
			new_page.merge_page(page,expand=False)
			

		fo_name = path[:-4] + "_E" + path[-4:] #attach _E at the end of the time, before extension, the file name is fixed and cannot be changed.
...
def mergeManager(list_of_files, output_name, add_blank_page_after_subfile,inherit_toc):
	...
	for z in list_of_files: # List of files
		with open(z,'rb') as f:    
			...
			m.append(tmp, outline_item = z[:-4], import_outline=inherit_toc) #Add outline
...
def tocManager(z,toc_structure):
	...
	with open(z,'rb') as f:
		...
		for y in toc_structure:
			y_params = {'title': y['title'], 'pagenum': y['pagenum'], 'parent': outline_root} 
		...
		with args.config[0].open() as f:
			...
			for i in range(0,len(lines)):
				...
				if current_indent_level==previous_indent_level:
					...
					if previous_indent_level==0: # if on the same indent level, and if indent level is root for example [Chapter 1], meets 'Chapter 2'
						config_outline_structure.append(temp_stack.pop()) #pop to root directory.
					...
					while current_indent_level < previous_indent_level:
						...
						if temp_entry['indent_level']==0:
							config_outline_structure.append(temp_entry) #only append to config_outline_structure if and only if it belongs to the root directory.
...
#def mergeManager(list_of_files, output_name, add_blank_pages_between_files,import_file_outlines):
		...
		default_output_name = default_output_name[:-4] + str('_M') + default_output_name[-4:]

		merge_args = {'list_of_files': valid_files, 'output_name': default_output_name, 'add_blank_page_after_subfile': args.add_blank_page_after_subfile, 'inherit_toc': args.inherit_toc}
